...
/*
	...
	docker的优点
		...
		2.响应式部署和扩展
			docker是基于容器的平台，允许高度可移植的工作负载。docker容器可以在开发人员
			的本机上，数据中心的物理或虚拟机上，云服务器上或混合环境中运行。
			
			docker的可移植性和轻量级的特性，还可以使您轻松地完成动态管理的工作负担，并
			根据业务需求指示，实时扩展或拆除应用程序或服务。

		3.在同一硬件上运行更多工作负载
			docker轻巧快速。它为基于虚拟机管理程序的虚拟机提供了可行、经济、高效的替代
			方案，因此您可以利用更多的计算能力来实现业务目标。docker非常适合于高度密集
			环境以及中小型部署，而您可以用更少的资源做更多的事情。
*/